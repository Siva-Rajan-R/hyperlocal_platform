...
def start_db_transaction(func):
    ...
    *Note, the first argument of the function, coroutine or object, should have a 'session' attribute or parameter.
    If a transaction is already active, uses a SAVEPOINT (nested transaction) instead of closing the session.
    """
    @wraps(func)
    async def async_wrapper(*args, **kwargs):
        ...
        if session.in_transaction() or session.in_nested_transaction():
            ic("Transaction already active → using SAVEPOINT (nested transaction)")
